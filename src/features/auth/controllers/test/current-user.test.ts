...
describe('CurrentUser', () => {
  ...
  describe('token', () => {
    it('Should set session token to null and send correct json reponse', async () => {
    ...
    it('Should set session token and send correct json response', async () => {
      const req: Request = authMockRequest({ jwt: '12345678' }, { username: USERNAME, password: PASSWORD }, authUserPayload) as Request;
      const res: Response = authMockResponse();
