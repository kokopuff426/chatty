...
export class MessageCache extends BaseCache {
  ...
  public async updateMessageReaction(
    ...
    try {
      ...
      const upadatedMessage: string = (await this.client.LINDEX(`messages:${conversationId}`, messageIndex)) as string;
      return Helpers.parseJson(upadatedMessage) as IMessageData;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again.');
    }
  ...
  private async getMessage(senderId: string, receiverId: string, messageId: string): Promise<IGetMessageFromCache> {
    ...
    const index: number = findIndex(messages, (listItem: string) => listItem.includes(messageId)) as number;
